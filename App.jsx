...
function Btn({ children, onClick, color="#3b82f6", outline, sm, full, disabled }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{padding:sm?"7px 14px":"10px 20px",borderRadius:10,border:outline?`1px solid ${color}`:"none",background:outline?"transparent":color,color:outline?color:"#fff",fontWeight:600,fontSize:sm?12:14,cursor:disabled?"not-allowed":"pointer",opacity:disabled?0.5:1,width:full?"100%":undefined,transition:"all .15s"}}>{children}</button>
  );
}
...
function TblHeader({ cols }) {
  return (
    <div style={{display:"grid",gridTemplateColumns:`repeat(${cols.length},1fr)`,padding:"10px 20px",background:"#141720",borderBottom:"1px solid #2a2d3e",fontSize:11,color:"#475569",fontWeight:700,letterSpacing:.5,gap:10}}>
      {cols.map((c,i)=><span key={i}>{c}</span>)}
    </div>
  );
}

function TblRow({ cols }) {
  const [hov,setHov]=useState(false);
  return (
    <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{display:"grid",gridTemplateColumns:`repeat(${cols.length},1fr)`,padding:"12px 20px",gap:10,borderBottom:"1px solid #1a1d2e",background:hov?"#1e2234":"transparent",transition:"background .15s",alignItems:"center"}}>
...
function WHCard({ wh, stock, onClick }) {
  const [hov,setHov]=useState(false);
  return (
    <div onClick={onClick} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      ...
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between"}}>
        ...
        <span style={{fontSize:10,color:wh.color,background:wh.color+"15",padding:"2px 8px",borderRadius:20,fontWeight:600}}>进入 →</span>
      </div>
...
function RecordsTable({ records, title, showWH, full }) {
  ...
  return (
    <div style={{background:"#1a1d2e",border:"1px solid #2a2d3e",borderRadius:16,overflow:"hidden"}}>
      ...
      <div style={full?{maxHeight:560,overflowY:"auto"}:{}}>
        ...
        {filtered.map((r,i)=>(
          <div key={r.id} style={{display:"grid",gridTemplateColumns:"80px 70px 100px 55px 1fr 1fr 85px",padding:"11px 20px",gap:8,borderBottom:i<filtered.length-1?"1px solid #141720":"none",alignItems:"center",fontSize:12}}>
            <span style={{color:"#475569"}}>{fmtDate(r.date)}</span>
            <span style={{fontWeight:600,fontSize:11,color:r.type==="in"?"#10b981":"#f59e0b",background:r.type==="in"?"rgba(16,185,129,.1)":"rgba(245,158,11,.1)",padding:"2px 6px",borderRadius:20,textAlign:"center"}}>{r.type==="in"?"⬇ 入库":"⬆ 出库"}</span>
            ...
            <span style={{color:"#64748b"}}>{r.supplier||"—"}{showWH&&r.whName&&<span style={{color:"#3b82f6",marginLeft:4}}>[{r.whName}]</span>}</span>
            <span style={{fontWeight:600,color:"#f1f5f9",textAlign:"right"}}>余{r.stockAfter}</span>
          </div>
        ))}
      </div>
    </div>
...
function WHModal({ form, setForm, onSave, onClose }) {
  return (
    <Modal title="添加仓库" onClose={onClose}>
      ...
      <div style={{marginTop:20}}><Btn full onClick={onSave} color={form.color}>确认添加</Btn></div>
    </Modal>
  );
}

function SKUModal({ form, setForm, onSave, onClose }) {
  return (
    <Modal title="添加杯子型号" onClose={onClose}>
      <Field label="型号编号 * （如：25001-1-C）"><Inp value={form.code} onChange={e=>setForm(f=>({...f,code:e.target.value}))} placeholder="25001-1-C"/></Field>
      ...
      <div style={{marginTop:20}}><Btn full onClick={onSave}>确认添加</Btn></div>
    </Modal>
  );
}
...
export default function App() {
  ...
  const [skuForm, setSkuForm] = useState({code:"",name:"",unit:"只"});
  const [txForm, setTxForm] = useState({type:"in",whId:"",skuId:"",qty:"",note:"",supplier:""});
  ...
  const addSKU = () => {
    ...
    setSkuForm({code:"",name:"",unit:"只"}); setModalSKU(false); showToast("型号添加成功 ✓");
  };
  ...
  const submitTx = async () => {
    ...
    await set(ref(db,`records/${recId}`),{id:recId,date:new Date().toISOString(),type:txForm.type,whId:txForm.whId,whName:wh.name,skuId:txForm.skuId,skuCode:sku.code,skuName:sku.name,qty,note:txForm.note,supplier:txForm.supplier,stockAfter:newQty});
    setTxForm(f=>({...f,qty:"",note:"",supplier:""})); setModalTx(false);
  ...
  const tabs=[{key:"overview",icon:"📊",label:"总览"},{key:"warehouses",icon:"🏭",label:"仓库"},{key:"skus",icon:"🥤",label:"型号"},{key:"records",icon:"📋",label:"记录"}];
      ...
      <div style={{minHeight:"100vh",background:"#0c0e17",color:"#e8e8e8",fontFamily:"'Noto Sans SC','PingFang SC',sans-serif"}}>
        ...
        <main style={{flex:1,padding:"24px 28px",maxWidth:1200,width:"100%",margin:"0 auto",boxSizing:"border-box"}}>
          ...
          <div style={{background:"#1a1d2e",border:"1px solid #2a2d3e",borderRadius:16,overflow:"hidden",marginBottom:24}}>
            ...
            {skuList.map(sku=>{const qty=whStock[sku.id]||0;return <TblRow key={sku.id} cols={[<code style={{color:"#3b82f6",fontFamily:"monospace",fontSize:13}}>{sku.code}</code>,<span style={{color:"#e2e8f0"}}>{sku.name}</span>,<span style={{fontWeight:700,color:qty>0?"#f1f5f9":"#ef4444",fontSize:15}}>{qty.toLocaleString()}</span>,<span style={{color:"#64748b"}}>{sku.unit}</span>]}/>;})}
          </div>
          <RecordsTable records={recList.filter(r=>r.whId===selWH)} title={`${wh.name} 操作记录`} showWH={false}/>
        </main>
    ...
    <div style={{minHeight:"100vh",background:"#0c0e17",color:"#e8e8e8",fontFamily:"'Noto Sans SC','PingFang SC',sans-serif",display:"flex",flexDirection:"column"}}>
      <header style={{background:"#12151f",borderBottom:"1px solid #1e2235",padding:"0 28px",display:"flex",alignItems:"center",justifyContent:"space-between",height:60,position:"sticky",top:0,zIndex:200}}>
        ...
        <nav style={{display:"flex",gap:2}}>
          {tabs.map(t=><button key={t.key} onClick={()=>setTab(t.key)} style={{padding:"6px 14px",borderRadius:8,border:"none",cursor:"pointer",fontSize:12,fontWeight:tab===t.key?700:400,background:tab===t.key?"linear-gradient(135deg,#1d4ed8,#0891b2)":"transparent",color:tab===t.key?"#fff":"#64748b",transition:"all .2s"}}>{t.icon} {t.label}</button>)}
        </nav>
      ...
      <main style={{flex:1,padding:"24px 28px",maxWidth:1200,width:"100%",margin:"0 auto",boxSizing:"border-box"}}>
        {tab==="overview"&&(
          <div>
            ...
            <div style={{marginBottom:14}}><div style={{fontSize:16,fontWeight:700,color:"#f1f5f9"}}>仓库概览</div><div style={{fontSize:11,color:"#475569",marginTop:2}}>点击进入仓库详情</div></div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:14,marginBottom:24}}>
              {whList.map(wh=><WHCard key={wh.id} wh={wh} stock={totalStock(wh.id)} onClick={()=>setSelWH(wh.id)}/>)}
            ...
            <div style={{background:"#1a1d2e",border:"1px solid #2a2d3e",borderRadius:16,overflow:"hidden",marginBottom:24}}>
              <TblHeader cols={["型号编号","名称","全仓库合计","单位"]}/>
              {skuList.length===0&&<Empty text="暂无型号"/>}
              {skuList.map(sku=>{const total=skuStockAll(sku.id);return <TblRow key={sku.id} cols={[<code style={{color:"#3b82f6",fontFamily:"monospace"}}>{sku.code}</code>,<span style={{color:"#e2e8f0"}}>{sku.name}</span>,<span style={{fontWeight:700,color:total>0?"#10b981":"#ef4444",fontSize:15}}>{total.toLocaleString()}</span>,<span style={{color:"#64748b"}}>{sku.unit}</span>]}/>;} )}
            </div>
            <RecordsTable records={recList.slice(0,8)} title="最近操作" showWH/>
          </div>
        )}
        {tab==="warehouses"&&(
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><div style={{fontSize:16,fontWeight:700,color:"#f1f5f9"}}>仓库管理</div><Btn onClick={()=>setModalWH(true)}>+ 添加仓库</Btn></div>
        ...
        {tab==="skus"&&(
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><div style={{fontSize:16,fontWeight:700,color:"#f1f5f9"}}>型号管理</div><Btn onClick={()=>setModalSKU(true)}>+ 添加型号</Btn></div>
            <div style={{background:"#1a1d2e",border:"1px solid #2a2d3e",borderRadius:16,overflow:"hidden"}}>
              <TblHeader cols={["型号编号","名称","单位",...whList.map(w=>w.name),"合计"]}/>
              {skuList.length===0&&<Empty text="暂无型号"/>}
              {skuList.map(sku=><TblRow key={sku.id} cols={[<code style={{color:"#3b82f6",fontFamily:"monospace",fontSize:13}}>{sku.code}</code>,<span style={{color:"#e2e8f0"}}>{sku.name}</span>,<span style={{color:"#64748b"}}>{sku.unit}</span>,...whList.map(wh=>{const q=data.stock[wh.id]?.[sku.id]||0;return <span style={{fontWeight:600,color:q>0?"#f1f5f9":"#475569"}}>{q}</span>;}),<span style={{fontWeight:700,color:"#3b82f6"}}>{skuStockAll(sku.id)}</span>]}/>)}
            </div>
          </div>
        )}
        {tab==="records"&&<RecordsTable records={recList} title="全部记录" showWH full/>}
      </main>
      {modalWH&&<WHModal form={whForm} setForm={setWhForm} onSave={addWarehouse} onClose={()=>setModalWH(false)}/>}
      {modalSKU&&<SKUModal form={skuForm} setForm={setSkuForm} onSave={addSKU} onClose={()=>setModalSKU(false)}/>}
      {modalTx&&<TxModal form={txForm} setForm={setTxForm} warehouses={whList} skus={skuList} stock={data.stock} onSave={submitTx} onClose={()=>setModalTx(false)}/>}
